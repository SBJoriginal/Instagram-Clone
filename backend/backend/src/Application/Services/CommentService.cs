...
namespace Application.Services
{
  public class CommentService : ICommentService
  {
    ...
    public async Task<CommentDto> AddCommentAsync(string userId, CreateCommentDto createCommentDto)
    {
      ...
      var image = await _context.Images.FirstOrDefaultAsync(i => i.Id == createCommentDto.ImageId);
      if (image != null)
      {
      ...
      var userProfile = await _context
        .UserProfiles.Where(p => p.UserId == userId)
    ...
    public async Task<IEnumerable<CommentDto>> GetCommentsByImageIdAsync(
      int imageId,
      int page = 1,
      int pageSize = 10
    )
    {
      var comments = await _context
        .Comments.Where(c => c.ImageId == imageId)
